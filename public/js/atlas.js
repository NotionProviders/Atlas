...
(function(){let s=11;function r(){s=(s*9301+49297)%233280;return s/233280;}
  for(let i=0;i<170;i++){const c=document.createElementNS(SVGNS,'circle');
    ...
    c.setAttribute('r',(r()*1.2+.35).toFixed(2));c.setAttribute('fill','#fff');
    c.setAttribute('opacity',(r()*.65+.25).toFixed(2));gStars.appendChild(c);}
})();
...
nodes.forEach(function(n){
  if(n.parent){
    ...
    shadow.setAttribute('x1',n.parent.x);shadow.setAttribute('y1',n.parent.y);shadow.setAttribute('x2',n.x);shadow.setAttribute('y2',n.y);
    shadow.setAttribute('class','edge-shade');shadow.setAttribute('stroke','#060a14');
    shadow.setAttribute('stroke-width',n.depth<=1?1.85:n.depth===2?1.45:1.15);
    shadow.setAttribute('stroke-linecap','round');shadow.setAttribute('vector-effect','non-scaling-stroke');
    gEdges.appendChild(shadow);n._edgeSh=shadow;
...
nodes.forEach(function(n){
  ...
  const shade=document.createElementNS(SVGNS,'circle');shade.setAttribute('class','node-shade');
...
function fade(n,v){
  if(searchActive)return n._match?v:v*0.45;
  if(n._ctx)return v;
  return Math.max(0.62,v*0.78);
}
...
function edgeRev(n,k){
  ...
  const rk=n.parent._ring*k;
  const ON=94,OFF=86;
  if(n._edgeHold==null)n._edgeHold=rk>ON;
  else if(rk>=ON)n._edgeHold=true;
  else if(rk<=OFF)n._edgeHold=false;
  if(!n._edgeHold)return 0;
  const r=smooth(rk,88,198);
  return r>0.96?1:r;
}
function edgeOnScreen(n,k,tx,ty,W,H,c,s){
  const p=n.parent;
  const rx1=p.x*c-p.y*s,ry1=p.x*s+p.y*c,sx1=rx1*k+tx,sy1=ry1*k+ty;
  const rx2=n.x*c-n.y*s,ry2=n.x*s+n.y*c,sx2=rx2*k+tx,sy2=ry2*k+ty;
  return Math.max(sx1,sx2)>-80&&Math.min(sx1,sx2)<W+80&&Math.max(sy1,sy2)>-80&&Math.min(sy1,sy2)<H+80;
}
function updateEdge(n,k,tx,ty,W,H,c,s){
  ...
  const base=n.depth<=1?0.72:n.depth===2?0.58:0.46;
  const eo=fade(n,er*base);
  n._edge.setAttribute('stroke-opacity',eo.toFixed(3));
  if(n._edgeSh)n._edgeSh.setAttribute('stroke-opacity',(eo*0.32).toFixed(3));
}
function frame(){
  ...
  for(let qi=0;qi<nodes.length;qi++){const n=nodes[qi];
    ...
    if(n._shade)n._shade.setAttribute('opacity',(body*0.28).toFixed(3));
...
function closeCtrlMenu(){
  if(ctrlMore.classList.contains('open')){
    ctrlMore.classList.remove('open');
    ctrlToggle.setAttribute('aria-expanded','false');
  }
}
function openCtrlMenu(){
  ctrlMore.classList.add('open');
